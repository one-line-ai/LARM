...
struct ActivityView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("AI 도구 활동 (Claude Code)").font(AppFont.title2)
            hookBox
            HStack {
                Picker("단계", selection: $phase) {
                    Text("요청").tag("request"); Text("결과").tag("result"); Text("작업 세션").tag("session_start"); Text("시험").tag("test"); Text("전체").tag("all")
                }.frame(maxWidth: 320)
                Toggle("위험 표시된 활동 기록만", isOn: $onlyRisk)
                Spacer()
            ...
            HSplitView {
                Table(items, selection: $selected) {
                    ...
                    TableColumn("판단") { (e: RuntimeEvent) in
                        if let r = e.riskRule { Text("\(r) \(e.riskOutcome == "unknown" ? "보류" : (e.riskSeverity ?? ""))").foregroundStyle(e.riskSeverity == "high" ? Theme.indigo : Theme.indigoSoft) } else { Text("") }
                    }.width(90)
        ...
        GroupBox("연동 상태") {
            VStack(alignment: .leading, spacing: 6) {
                if let m = state.monitor {
                    HStack {
                        Text("Claude Code 연결(hook): ").bold()
                        Text(m.hookInstalled ? "등록됨" : "미등록")
                        Text("· 수신 연결 통로: \(m.health.activity.label)")
                        if let v = m.activityVerifiedAt { Text("· 연결 확인 신호 확인 \(Fmt.elapsed(Clock.utc(v)))").foregroundStyle(Theme.inkSoft) }
                        if let l = m.lastEventAt { Text("· 마지막 수신 \(Fmt.elapsed(Clock.utc(l)))").foregroundStyle(Theme.inkSoft) }
                    }
                    HStack {
                        if m.hookInstalled { Button("연결 제거 (LARM 항목만)") { state.prepareHookInstall(remove: true) } }
                        else { Button("연결 등록 (변경 전후 확인)") { state.prepareHookInstall(remove: false) } }
                        Button("연결 확인 신호 보내기") { state.sendTestEvent() }
                    }
                    Text("등록은 ~/.claude/settings.json의 hooks에 LARM 항목(PreToolUse, PostToolUse, SessionStart, SessionEnd)만 추가함. 이 연결은 허용이나 차단 결정을 하지 않고 기록만 함. 앱이 꺼져 있으면 보관했다가 다음 실행 때 읽음. Codex와 Cursor의 활동은 지원하지 않음")
                        .font(AppFont.footnote).foregroundStyle(Theme.inkSoft)
                    if let msg = state.hookMessage { Text(msg).font(AppFont.footnote).foregroundStyle(Theme.indigoSoft).textSelection(.enabled) }
                }
    ...
    var detail: some View {
        ScrollView {
            if let id = selected, let e = state.events.first(where: { $0.eventID == id }) {
                VStack(alignment: .leading, spacing: 8) {
                    Text("\(e.toolName) · \(e.phase)").font(AppFont.headline)
                    ...
                    if let b = e.commandBasename { LabeledContent("명령 종류", value: "\(b) · 명령 내용 \(e.argc)개") }
                    LabeledContent("표시", value: e.flags.isEmpty ? "없음" : e.flags.joined(separator: ", "))
                    LabeledContent("명령 내용 요약값", value: String(e.argFingerprint.prefix(16)) + "…")
                    LabeledContent("작업 세션 / 프로세스", value: "\(e.sessionRef.prefix(8))… / \(e.processRef)")
                    ...
                    GroupBox("요청과 결과") {
                        VStack(alignment: .leading) {
                            ForEach(linked) { l in Text("\(l.phase) · \(Fmt.local(l.observedAt))\(l.phase == "result" ? " · 결과 기록 있음 (성공 여부는 도구 응답 기준 미해석)" : "")").font(AppFont.footnote) }
                            if !linked.contains(where: { $0.phase == "result" }) && e.phase == "request" {
                                Text("결과 활동 기록 없음: 실행 여부 미확인. 실행 성공으로 표시하지 않음").font(AppFont.footnote).foregroundStyle(Theme.indigoSoft)
                            }
                        }.frame(maxWidth: .infinity, alignment: .leading)
                    }
                    if let r = e.riskRule {
                        GroupBox("판단 \(r) (점검 규칙 \(e.riskRule ?? "") v1.0.0)") {
                            VStack(alignment: .leading, spacing: 4) {
                                Text(e.riskOutcome == "unknown" ? "판단 보류" : "심각도 \(e.riskSeverity ?? "")").bold()
                                Text(e.riskSummary ?? "")
                                Text("한계: \(e.riskLimits ?? "")").font(AppFont.footnote).foregroundStyle(Theme.inkSoft)
                                Text("다음 단계: \(e.riskNext ?? "")").font(AppFont.footnote)
                            }.frame(maxWidth: .infinity, alignment: .leading)
                        }
                        HStack {
                            if e.ackState == "observed" { Button("확인함") { state.acknowledge(e, state: "acknowledged") } }
                            if e.ackState != "false_positive_review" { Button("잘못된 탐지 검토") { state.acknowledge(e, state: "false_positive_review") } }
                            Text("확인해도 기록은 그대로 남음.").font(AppFont.footnote).foregroundStyle(Theme.inkSoft)
                        }
                    }
                }.padding(8).frame(maxWidth: .infinity, alignment: .leading)
            } else { Text("선택한 활동 기록 없음").foregroundStyle(Theme.inkSoft).padding() }
        }
    }
}
...
struct HookPlanSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(state.hookPlanIsRemoval ? "LARM 연결(hook) 항목 제거: 변경 전후" : "Claude Code 연결 등록: 변경 전후").font(AppFont.headline)
            Text("파일: ~/.claude/settings.json · 기존 연결(hook)과 다른 설정은 그대로 둠. 적용은 임시 파일 + rename으로 원자적으로 수행함").font(AppFont.footnote).foregroundStyle(Theme.inkSoft)
