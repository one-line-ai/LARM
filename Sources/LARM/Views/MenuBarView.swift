...
struct MenuBarView: View {
    ...
    var body: some View {
        ...
        case .ready:
            ...
            if let ok = state.monitor?.health.lastOKAt { Text("감시 동작 확인 \(Fmt.elapsed(Clock.utc(ok)))") }
            Text("미조치 높은 위험 \(state.openHighCount) · 확인 못 한 항목 \(state.gapCount) · 감시가 끊긴 구간 \(state.watchGapCount)")
