...
public enum Exporter {
    ...
    public static func build(_ i: ExportInput, now: String = Clock.nowUTC()) throws -> (Data, String, String) {
        ...
        files.append(("diff.json", try StableJSON.data(["schema": "larm-diff/1.1", "scan_id": i.scanID, "baseline_scan_id": orNull(i.baselineScanID),
            "note": i.baselineScanID == nil ? "기준점이 없어 비교하지 않았습니다" : "기준점 대비 비교. 읽기 실패·범위 제거는 삭제가 아니라 비교 불가입니다",
        ...
        files.append(("events.json", try StableJSON.data(["schema": "larm-events/1.1", "note": "Claude Code hook 확인 이벤트 (확인 전용). 원문 prompt·인자 제외. 항목 없음은 미확인을 뜻하며 안전을 뜻하지 않는다.",
        ...
        files.append(("coverage_gaps.json", try StableJSON.data(["schema": "larm-coverage-gaps/1.1", "note": "감시 공백. 계획 중지·장애·유실을 구분하며 공백 중 행위는 미확인이다.",
        ...
        files.append(("action_results.json", try StableJSON.data(["schema": "larm-action-results/1.1", "note": "집행 결과 없음: 실행 전 확인·정책 집행(F42/F43)은 제공하지 않는다 (확인 모드). 결과 이벤트 존재는 events.json의 result_present로만 표시한다.",
    ...
    static func summaryHTML(_ i: ExportInput, exportID: String, now: String, scopeAlias: [String: String]) -> String {
        ...
        var h = """
        <!DOCTYPE html><html lang="ko"><head><meta charset="utf-8"><title>LARM 증빙 요약</title>
        ...
        \(i.scanSummary.status != .complete ? "<p class=\"warn\">부분 결과입니다. 전체 완료로 해석하지 마세요.</p>" : "")
        <h2>집계</h2>
        <table><tr><th>열린 위험</th><th>조치 중</th><th>재점검 해소</th><th>예외</th><th>오탐 검토</th><th>점검 공백</th><th>변경 항목</th></tr>
        ...
        <p>위험과 공백을 가중 합산한 단일 점수는 만들지 않습니다.</p>
        ...
        h += "</table><h2>예외 (위험 수용)</h2><table><tr><th>룰</th><th>위치</th><th>만료</th><th>사유</th></tr>"
        ...
        h += "</table><h2>점검 공백</h2><table><tr><th>어댑터</th><th>항목</th><th>상태</th><th>사유</th></tr>"
        for c in gaps { h += "<tr><td>\(esc(c.adapter))</td><td>\(esc(c.itemAlias))</td><td>\(esc(c.status.rawValue))</td><td>\(esc(c.reason))</td></tr>" }
        h += "</table><h2>기준점 대비 변경</h2>"
        if let b = i.baselineScanID {
            h += "<p>기준점 점검 \(esc(b)) 대비 \(i.diff.count)건 (추가 \(i.diff.filter { $0.change == .added }.count) · 삭제 \(i.diff.filter { $0.change == .removed }.count) · 수정 \(i.diff.filter { $0.change == .modified }.count) · 비교 불가 \(i.diff.filter { $0.change == .incomparable }.count))</p>"
        } else { h += "<p>기준점이 없어 비교하지 않았습니다.</p>" }
        h += """
        <h2>검증 한계</h2>
        <ul><li>정적 설정 확인이며 실제 실행·통신·유출을 확정하지 않습니다.</li>
        <li>비밀정보 후보는 형식 판단이며 유효성은 미확인입니다. 원문 값은 포함하지 않습니다.</li>
        <li>파일 검증 성공은 묶음 내부 일관성이며 생성자 신원·시점 공증이 아닙니다.</li>
        <li>활동 이벤트은 Claude Code hook이 등록된 세션의 요청·결과 이벤트만 담습니다 (\(i.events.count)건). 집행 결과는 없습니다 (확인 모드).</li>
        <li>감시 공백 \(i.gaps.count)건 기록. 공백 중 행위는 미확인입니다.</li></ul>
        </body></html>
        """
        return h
    }
    ...
    static func readme(exportID: String) -> String {
        """
        LARM 증빙 묶음 (\(schema)) · export_id \(exportID)

        [오프라인 검증 절차]
        1. 인터넷·LARM 계정 없이 larm-verify 를 실행합니다:
             /Applications/LARM.app/Contents/MacOS/larm-verify <이 파일.zip>
           또는 수동으로: manifest.json 의 files[] 에 적힌 경로·크기·SHA-256 을 각 파일과 대조합니다 (shasum -a 256).
        2. 성공은 "manifest 에 적힌 파일과 묶음의 파일이 일치함" 입니다. 생성자 신원, 확인 사실의 진실성, 시점 공증, 악성 여부, 법규 준수를 증명하지 않습니다.
        3. manifest 까지 재작성한 위조는 별도 매체에 보관한 manifest SHA-256 과 대조해야 검출됩니다 (--expect-manifest-sha256).

        [호환 버전] schema larm-evidence/1.1 (1.0 정적 증빙과 구분). ZIP 은 저장 방식(무압축)만 사용합니다.

        [개인정보 취급] 원문 토큰·비밀값·raw args·URL query·사용자명·기기 일련번호를 포함하지 않습니다. 경로는 별칭입니다.
        내보낸 사본은 앱의 보존 정책과 별개이며 수신자가 관리합니다.
